...
void updateShape(Shape *s, double theta) {
  updateVector(s->pos, theta);
  updateRotation(s->rotation, theta);
}

void renderShape(Shape *s) {
  switch (s->type) {
  case RectangleShape:
    drawRectangle(s->pos->pos, s->rotation->rot, s->size->x, s->size->y,
                  s->color);
    break;
  case CircleShape:
...
void setShapeRotation(Shape *s, Rotation *r) {
  if (s->rotation == NULL)
    free(s->rotation);
  s->rotation = r;
}
